  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="job group name" prop="jobGroup">
        <el-select v-model="queryParams.jobGroup" placeholder="please select job group name" clearable>
          <el-option
          ...
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['monitor:job:changeStatus', 'monitor:job:query']">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="handleJobLog" icon="el-icon-s-operation"
                v-hasPermi="['monitor:job:query']">job log</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-row>
            ...
            <el-form-item prop="invokeTarget">
              <span slot="label">
                ...
                <el-tooltip placement="top">
                  <div slot="content">
                    Bean invoke example: AppTask.appParams('app')
                    <br />Class invoke example: com.app.quartz.task.AppTask.appParams('app')
                    <br />Parameter definition: support string, bool, long integer, float, integer
                  </div>
          ...
          <el-col :span="24">
            <el-form-item label="execute strategy" prop="misfirePolicy">
              <el-radio-group v-model="form.misfirePolicy" size="small">
                <el-radio-button label="1">immidiate</el-radio-button>
                <el-radio-button label="2">execute once</el-radio-button>
                <el-radio-button label="3">quit execute</el-radio-button>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="enable concurrent" prop="concurrent">
      ...
      <el-form ref="form" :model="form" label-width="120px" size="mini">
        <el-row>
          ...
          <el-col :span="12">
            ...
            <el-form-item label="date of creation:">{{ form.createTime }}</el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="enable concurrent:">
              <div v-if="form.concurrent == 0">Yes</div>
              <div v-else-if="form.concurrent == 1">No</div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="execute strategy:">
              <div v-if="form.misfirePolicy == 0">default</div>
              <div v-else-if="form.misfirePolicy == 1">immidiate</div>
              <div v-else-if="form.misfirePolicy == 2">execute once</div>
              <div v-else-if="form.misfirePolicy == 3">quit execute</div>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        jobName: [
          { required: true, message: "job name can not be empty", trigger: "blur" }
        ],
        invokeTarget: [
          { required: true, message: "invoke target string can not be empty", trigger: "blur" }
        ],
        cronExpression: [
          { required: true, message: "cron expression can not be empty", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      let text = row.status === "0" ? "enabled" : "disabled";
      this.$modal.confirm('sure"' + text + '""' + row.jobName + '"job?').then(function() {
    ...
    handleRun(row) {
      this.$modal.confirm('sure execute once"' + row.jobName + '"job?').then(function() {
        return runJob(row.jobId, row.jobGroup);
      }).then(() => {
        this.$modal.msgSuccess("execute success");
      }).catch(() => {});
    },
    handleView(row) {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('sure delete job id"' + jobIds + '"data?').then(function() {
