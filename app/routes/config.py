...
def sys_config_list():
    ...
    return jsonify({'msg': 'operation success', 'code': 200, 'rows': [config.to_json() for config in config_list], 'total': pagination.total})
...
def sysconfig_get_by_id(id):
    ...
    return jsonify({'msg': 'operation success', 'code': 200, 'data': config.to_json()})
...
def config_export():
    rows = []
    rows.append(['parameter primary key', 'parameter name', 'parameter key name', 'parameter key value', 'system built-in', 'comment', 'date of creation'])
