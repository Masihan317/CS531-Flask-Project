...
def do_login():
    ...
    if user is not None:
        ...
        if md.hexdigest() == user.PWD:
            ...
            return jsonify({'msg': 'login success~', 'code': 200, 'url': '/', 'token': str(uuid.uuid4())})
    return jsonify({'msg': 'login fail, wrong account/password~', 'code': 500})
...
def user_grid():
    ...
    return jsonify({'rows': [user.to_json() for user in users], 'total': pagination.total, 'code': 200, 'msg': 'lookup success'})
...
def syuser_save():
    if User.query.filter_by(LOGINNAME = request.json['userName']).first():
        return jsonify({'success': False, 'msg': 'add user acount fail, account already exist!'})
    ...
    return jsonify({'code': 200, 'msg': 'add user account success!'})
...
def syuser_update_pwd():
    ...
    if user:
        ...
        if md.hexdigest() != user.PWD:
            return jsonify({'code': 400, 'msg': 'wrong old password'})

        md = hashlib.md5()
        md.update(request.args.get('newPassword').encode('utf-8'))
        user.PWD = md.hexdigest()
        db.session.add(user)
    return jsonify({'code': 200, 'msg': 'modify success'})
...
def syuser_info():
    ...
    return jsonify({'msg': 'login success~', 'code': 200, \
...
@login_required
def syuser_profile():
     return jsonify({'msg': 'operation success', 'code': 200, \
...
def syuser_auth_role(id):
    ...
    return jsonify({'code': 200, 'msg': 'operation success', 'roles': [role.to_json() for role in allRoles], 'user': user.to_json()})

@base.route('/base/syuser/export', methods=['POST'])
@login_required
def user_export():
    rows = []
    rows.append(['login name', 'name', 'date of creation', 'date of updated', 'gender'])
